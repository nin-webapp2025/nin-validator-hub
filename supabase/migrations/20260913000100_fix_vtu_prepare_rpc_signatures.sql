-- Align VTU purchase RPC signatures with the deployed edge functions.
-- The edge functions pass p_service_identifier for receipts; production may
-- still have older RPC signatures if same-day migrations were skipped.
...
DROP FUNCTION IF EXISTS public.prepare_vtu_purchase(uuid, uuid, text, text, numeric, text);
...
BEGIN
  ...
  IF v_operation IS NULL THEN
    RETURN jsonb_build_object('success', false, 'message', 'This bill payment category is unsupported.');
  END IF;
